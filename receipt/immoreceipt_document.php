...
/**
 *  \file       immoreceipt_document.php
 *  \ingroup    ultimateimmo
 *  \brief      Tab for documents linked to ImmoReceipt
 */
...
if ($id > 0 || !empty($ref)) $upload_dir = $conf->ultimateimmo->multidir_output[$object->entity ? $object->entity : $conf->entity] . "/receipt/" . dol_sanitizeFileName($object->ref);

$permissiontoadd = $user->hasRight('ultimateimmo','owner','write'); // Used by the include of actions_addupdatedelete.inc.php
...
$title = $langs->trans("ImmoReceipt") . ' - ' . $langs->trans("Files");
$help_url = '';
...
if ($object->id) {
	...
	print dol_get_fiche_head($head, 'document', $langs->trans("ImmoReceipt"), -1, 'bill');
	...
	$morehtmlref .= $form->editfieldkey("RefCustomer", 'ref_client', $object, $staticImmorenter->ref, 0, 'string', '', 0, 1);
	$morehtmlref .= $form->editfieldval("RefCustomer", 'ref_client', $staticImmorenter->ref . ' - ' . $staticImmorenter->getFullName($langs), $object, 0, 'string', '', null, null, '', 1);
	// Thirdparty
	...
	//$permission = $user->hasRight('ultimateimmo','receipt','write');
	$permission = 1;
	//$permtoedit = $user->hasRight('ultimateimmo','receipt','write');
	$permtoedit = 1;
	$param = '&id=' . $object->id;

	//$relativepathwithnofile='immoreceipt/' . dol_sanitizeFileName($object->id).'/';
	$relativepathwithnofile = 'receipt/' . dol_sanitizeFileName($object->ref) . '/';
