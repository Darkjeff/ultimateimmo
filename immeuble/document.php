...
/**
 * \file htdocs/custom/immobilier/local/document.php
 */
...
$object = new Immeuble ( $db );
$object->fetch ( $id );

$upload_dir = $conf->immobilier->dir_output . '/immeuble/' . dol_sanitizeFileName ( $object->id );
...
if ($object->id) {
	...
	print '<tr><td width="20%">' . $langs->trans ( "NomImmeuble" ) . '</td>';
	print '<td><input name="nom" size="40" value="' . $immeuble->nom . '"</td></tr>';
	...
	$formfile->form_attach_new_file ( DOL_URL_ROOT . '/immobilier/immeuble/document.php?id=' . $object->id, '', 0, 0 );
