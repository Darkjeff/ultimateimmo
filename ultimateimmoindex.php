...
/**
 *	\file       htdocs/ultimateimmo/ultimateimmoindex.php
...
llxHeader("", $langs->trans("UltimateimmoArea"), $wikihelp);
...
if ($result) {
	...
	while ($i < $num) {
		...
		if ($objp->status == -1) {
			$OwnerToValidate[$objp->rowid] = $objp->somme;
		}
		if ($objp->status == 1) {
			$OwnersValidated[$objp->rowid] = $objp->somme;
		}
		if ($objp->status == 0) {
			$OwnersResiliated[$objp->rowid] = $objp->somme;
		}
