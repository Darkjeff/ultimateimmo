...
class pdf_chargefourn extends ModelePDFImmobilier {
	...
	function write_file($year, $outputlangs, $filedir, $filename) {
		...
		if (file_exists($dir)) {
			...
			$sql .= " INNER JOIN " . MAIN_DB_PREFIX . "immo_local as ll ON ic.local_id = ll.rowid";
			$sql .= " INNER JOIN " . MAIN_DB_PREFIX . "immo_property as ii ON ll.immeuble_id = ii.rowid";
			$sql .= " INNER JOIN " . MAIN_DB_PREFIX . "immo_typologie as it ON it.rowid = ic.type AND ic.type=23";
