...
if (! empty($conf->accounting->enabled)) { include_once DOL_DOCUMENT_ROOT . '/core/class/html.formaccounting.class.php';
}
                ...
                if ($action == 'edit' && ($rowid == (! empty($obj->rowid)?$obj->rowid:$obj->code))) {
                    ...
                    if (empty($reshook)) {
                        ...
                        foreach ($fieldlist as $field => $value)
                        {
                            ...
                            if ($showfield && $fieldlist[$field] != 'code') { print '<!-- '.$fieldlist[$field].' --><td align="'.$align.'" class="'.$class.'">'.$valuetoshow.'</td>';
                            }
                        }
                    }
                    ...
                    if ($iserasable) {
                        ...
                        if ($user->rights->gestionparc->dict) { print '<a href="'.$url.'action=delete">'.img_delete().'</a>';
                        }
...
function fieldList($fieldlist, $obj = '', $tabname = '', $context = '')
{
    ...
    foreach ($fieldlist as $field => $value)
    {
        ...
        if (in_array($fieldlist[$field], array('code', 'libelle', 'type')) && $tabname == MAIN_DB_PREFIX."c_actioncomm" && in_array($obj->type, array('system','systemauto'))) {
            ...
            $classtd=''; $class='';
            if ($fieldlist[$field]=='libelle' || $fieldlist[$field]=='label') { $classtd='centpercent';
            }
            ...
            if (! $transfound && $fieldlist[$field]!='code') {
