...
/**
 *   	\file       immocompteur_list.php
 *		\ingroup    ultimateimmo
 *		\brief      List page for immocompteur
 */
...
$contextpage = GETPOST('contextpage', 'aZ') ? GETPOST('contextpage', 'aZ') : 'immocompteurlist'; // To manage different context of search
...
$hookmanager->initHooks(array('immocompteurlist')); // Note that conf->hooks_modules contains array
...
if (isset($extrafields->attributes[$object->table_element]['label'])
	...
	&& count($extrafields->attributes[$object->table_element]['label']) > 0) {
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'ImmoCompteur';
	$objectlabel = 'ImmoCompteur';
...
//$help_url="EN:Module_ImmoCompteur|FR:Module_ImmoCompteur_FR|ES:Módulo_ImmoCompteur";
$help_url = '';
$title = $langs->trans('ListOf', $langs->transnoentitiesnoconv("ImmoCompteurs"));
...
if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/ultimateimmo/compteur/immocompteur_card.php', 1).'?id='.$id);
	exit;
}
...
$newcardbutton = dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/ultimateimmo/compteur/immocompteur_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendImmoCompteurRef";
$modelmail = "immocompteur";
$objecttmp = new ImmoCompteur($db);
...
foreach ($object->fields as $key => $val) {
	...
	if (!empty($arrayfields['t.'.$key]['checked'])) {
		...
		elseif (strpos($val['type'], 'integer:') === 0) {
			print $object->showInputField($val, $key, (isset($search[$key])?$search[$key]:null), '', '', 'search_', 'maxwidth150', 1);
		} elseif (!preg_match('/^(date|timestamp)/', $val['type'])) print '<input type="text" class="flat maxwidth75" name="search_'.$key.'" value="'.(isset($search[$key])?dol_escape_htmltag($search[$key]):'').'">';
		print '</td>';
	}
}
// Extra fields
