...
/**
 *   	\file       immocompteur_card.php
 *		\ingroup    ultimateimmo
 *		\brief      Page to create/edit/view immocompteur
 */
...
$contextpage = GETPOST('contextpage', 'aZ') ?GETPOST('contextpage', 'aZ') : 'immocompteurcard'; // To manage different context of search
...
if (empty($reshook))
{
	...
	if (empty($backtopage) || ($cancel && empty($id))) {
		if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
			...
			else $backtopage = dol_buildpath('/ultimateimmo/compteur/immocompteur_card.php', 1).'?id='.($id > 0 ? $id : '__ID__');
		}
	}
	...
	{
		$object->setValueFrom('fk_soc', GETPOST('fk_soc', 'int'), '', '', 'date', '', $user, 'IMMOCOMPTEUR_MODIFY');
	}
	...
	$triggersendname = 'IMMOCOMPTEUR_SENTBYMAIL';
	$autocopy = 'MAIN_MAIL_AUTOCOPY_IMMOCOMPTEUR_TO';
	$trackid = 'immocompteur'.$object->id;
...
if ($action == 'create')
{
	print load_fiche_titre($langs->trans("NewObject", $langs->transnoentitiesnoconv("ImmoCompteur")), '', 'object_'.$object->picto);
...
{
	print load_fiche_titre($langs->trans("ImmoCompteur"), '', 'object_'.$object->picto);
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create')))
{
	...
	if ($action == 'clone') {
		...
		$formquestion = array();
		$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"].'?id='.$object->id, $langs->trans('CloneImmoCompteur'), $langs->trans('ConfirmCloneAsk', $object->ref), 'confirm_clone', $formquestion, 'yes', 1);
	}
	...
	// Object card
	// ------------------------------------------------------------
	$linkback = '<a href="'.dol_buildpath('/ultimateimmo/compteur/immocompteur_list.php', 1).'?restore_lastsearch_values=1'.(!empty($socid) ? '&socid='.$socid : '').'">'.$langs->trans("BackToList").'</a>';
	...
	if ($action != 'presend' && $action != 'editline') {
		...
		if (empty($reshook))
		{
			...
			if ($permissiontoadd)
			{
				print '<a class="butAction" href="'.$_SERVER['PHP_SELF'].'?id='.$object->id.'&socid='.$object->socid.'&action=clone&object=immocompteur">'.$langs->trans("ToClone").'</a>'."\n";
			}
	...
	if ($action != 'presend')
	{
		...
		$linktoelem = $form->showLinkToObjectBlock($object, null, array('immocompteur'));
		...
		$somethingshown = $formactions->showactions($object, $object->element.'@ultimateimmo', (is_object($object->thirdparty) ? $object->thirdparty->id : 0), 1, '', $MAXEVENT, '', $morehtmlright);
