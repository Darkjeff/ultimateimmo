...
class Local extends CommonObject {
	 ...
	 */
	function fetchAllByBuilding($activ = 'Actif') {
		global $user;
		
		$sql = "SELECT * ";
		$sql .= " FROM " . MAIN_DB_PREFIX . "immo_local as l";
		$sql .= " WHERE l.statut = '" . $activ . "'  ";
		$sql .= " AND l.immeuble_id = " . $this->immeuble_id;
		$sql .= " ORDER BY nom";
		...
		if ($resql) {
			...
			while ($obj = $this->db->fetch_object($resql)) {
				...
				$line->id = $obj->rowid;
				$line->immeuble_id = $obj->immeuble_id;
				$line->nom = $obj->nom;
				$line->adresse = $obj->adresse;
				$line->commentaire = $obj->commentaire;
				$line->statut = $obj->statut;
				$line->superficie = $obj->superficie;
				$line->type = $obj->type;
				$line->date_dpe = $obj->date_dpe;
				$line->dpe_ep = $obj->dpe_ep;
				$line->dpe_ges = $obj->dpe_ges;
	...
	function getNomUrl($withpicto = 0, $maxlen = 0) {
		...
		$lien = '<a href="' . DOL_URL_ROOT . '/custom/immobilier/local/fiche_local.php?action=update&id=' . $this->id . '">';
		$lienfin = '</a>';
		...
		if (file_exists($dir_osencoded)) {
			...
			if (is_resource($handle)) {
				while ( ($file = readdir($handle)) != false ) {
					...
					if (! utf8_check($file))
						$file = utf8_encode($file); // To be sure file is stored in UTF8 in memory
