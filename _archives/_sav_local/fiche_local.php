...
if (GETPOST ( "action" ) == 'add') {
	...
	if ($res == 0) {
		Header ( "Location: " . DOL_URL_ROOT . "/custom/immobilier/local.php" );
	} else {
		if ($res == - 3) {
			$_error = 1;
			$action = "create";
	...
	$res = $local->update ();
	header ( "Location: " . DOL_URL_ROOT . "/custom/immobilier/local/fiche_local.php?id=" . $local->id );
	if ($res >= 0) {
...
if ($action == 'create') {
	...
	print '<tr><td width="25%">'.$langs->trans("NomLocal").'</td>';
	...
	print '</td></tr>';
	print '<tr><td>'.$langs->trans("Adresse").'</td>';
