...
/**
 *   	\file       immorent_list.php
 *		\ingroup    ultimateimmo
 *		\brief      List page for immorent
 */
...
$contextpage = GETPOST('contextpage', 'aZ') ? GETPOST('contextpage', 'aZ') : 'immorentlist'; // To manage different context of search
...
$hookmanager->initHooks(array('immorentlist'));     // Note that conf->hooks_modules contains array
...
if ($enablepermissioncheck) {
	$permissiontoread = $user->rights->ultimateimmo->immorent->read;
	$permissiontoadd = $user->rights->ultimateimmo->immorent->write;
	$permissiontodelete = $user->rights->ultimateimmo->immorent->delete;
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'ImmoRent';
	$objectlabel = 'ImmoRent';
	$uploaddir = $conf->ultimateimmo->dir_output . '/rent/';
...
if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: " . dol_buildpath('/ultimateimmo/rent/immorent_card.php', 1) . '?id=' . $id);
	exit;
}
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/ultimateimmo/rent/immorent_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);
...
$topicmail = "SendImmoRentRef";
$modelmail = "immorent";
$objecttmp = new ImmoRent($db);
$trackid = 'rent' . $object->id;
			...
			if (!empty($arrayfields['t.' . $key]['checked'])) {
				...
				if ($key == 'status') {
					...
					if ($object->totalamount) {
						$total_amount = price($object->totalamount, 0, $outputlangs, 1, -1, -1, $conf->currency);
						print $total_amount;
					}
				} elseif ($val['label'] == 'ChargesAmount') {
					if ($object->chargesamount) {
						$chargesamount = price($object->chargesamount, 0, $outputlangs, 1, -1, -1, $conf->currency);
						print $chargesamount;
					}
...
// Show total line
//include DOL_DOCUMENT_ROOT.'/core/tpl/list_print_total.tpl.php'; // I don't use the template to add currency
// Show total line
