...
if (($id || $ref) && $action == 'edit') {
	print load_fiche_titre($langs->trans("TitreEditImmoCost"));
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
	...
	if ($action == 'clone') {
		...
		$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"] . '?id=' . $object->id, $langs->trans('CloneImmoCost'), $langs->trans('ConfirmCloneAsk', $object->ref), 'confirm_clone', $formquestion, 'yes', 1);
	}
	...
	*/
	...
	if ($action != 'presend') {
		...
		$morehtmlright = '<a href="' . dol_buildpath('/ultimateimmo/cost/immocost_agenda.php', 1) . '?id=' . $object->id . '">';
