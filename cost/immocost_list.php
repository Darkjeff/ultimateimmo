...
/**
 *   	\file       immocost_list.php
 *		\ingroup    immobilier
 *		\brief      List page for immocost
 */
...
$langs->loadLangs(array("immobilier@immobilier","other","main"));
...
$contextpage= GETPOST('contextpage','aZ')?GETPOST('contextpage','aZ'):'immocostlist';   // To manage different context of search
...
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."immocost_extrafields as ef on (t.rowid = ef.fk_object)";
if ($object->ismultientitymanaged == 1) $sql.= " WHERE t.entity IN (".getEntity('immocost').")";
else $sql.=" WHERE 1 = 1";
...
$newcardbutton='';
$newcardbutton='<a class="butActionNew" href="' .dol_buildpath('/immobilier/cost/immocost_card.php',1) . '?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']).'"><span class="valignmiddle">'.$langs->trans('New').'</span>';
...
$topicmail="SendImmoCostRef";
$modelmail="immocost";
$objecttmp=new ImmoCost($db);
...
if (isset($totalarray['pos']))
{
	...
	{
		$i++;
		if (! empty($totalarray['pos'][$i]))  print '<td align="left">'.price($totalarray['val'][$totalarray['pos'][$i]]).'</td>';
		else
		{
			if ($i == 1)
			{
