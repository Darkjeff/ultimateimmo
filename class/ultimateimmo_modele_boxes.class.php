...
/**
 * Class for Modele Boxes
 */
class UltimateImmoModeleBoxes
{
    public $db;
    ...
    public function fetch($rowid)
    {
        ...
        $sql.= " FROM ".MAIN_DB_PREFIX."c_ultimateimmo_boxes as b";
     ...
     * @param array     $styles         Array with properties of box style
     * @return string
     */
    public function showBox($head = null, $contents = null, $nooutput = 0, $styles = null)
    {
        ...
        if ($refresh) {
            ...
            {
                $out.= '<table summary="boxtable'.$this->box_id.'" width="100%" class="noborder boxtable" style="border-color:'.$backgroundcolor.';border-radius: .25rem;">'."\n";
            }

            // Show box title
            if (! empty($head['text']) || ! empty($head['sublink']) || ! empty($head['subpicto']))
            {
                $out.= '<tr class="liste_titre box_titre" style="background-color: '.$backgroundcolor.';color: '.$color.';">';
                $out.= '<td';
     ...
     * Function used by dashboard page. List is sorted by widget filename so by priority to run.
     *
     * @param array     $forcedirwidget     null=All default directories. This parameter is used by modulebuilder module only.
     * @return array
     */
    public function getWidgetsList($forcedirwidget = null)
    {
        global $langs, $db;
        ...
        foreach ($orders as $key => $value)
        {
            ...
            if (is_object($objMod))
            {
                ...
                if ($disabledbyname > 0) $widget[$j]['status'] = '';
