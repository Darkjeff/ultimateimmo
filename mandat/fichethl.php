...
require_once (DOL_DOCUMENT_ROOT."/gestimmo/class/mandat.class.php");
...
// ajouter un mandats 
//todo gestion des droit , par defaut ceux de contrat
...
/* $this->ref_interne = $obj->ref_interne;
	    ...
	    $object->rowid						= $rowid;
	    $object->ref_interne                = $ref_interne;
    	...
    	$object->fk_biens					= GETPOST('biensid','int');
    	
    	$object->date_cloture               = GETPOST('date_cloture');
        
    	$object->status = GETPOST('status');
               $object->mise_en_service = GETPOST('mise_en_service') ;
               $object->fin_validite = GETPOST('fin_validite');
               $object->fk_bails = getpost('bailid','int');
             //   $object->fk_commercial = GETPOST('commercial_suivi_id','int');
...
llxHeader('',$langs->trans("Mandat"),"Mandat");
...
if ($action == 'create')
{
    dol_fiche_head('', '', $langs->trans("AddMandat"), 0, 'Mandat');
...
/* *************************************************************************** */
{
    ...
    if (! empty($object->brouillon) && $user->rights->contrat->creer)
        ...
        $linkback = '<a href="'.DOL_URL_ROOT.'/mandat/liste.php'.(! empty($socid)?'?socid='.$socid:'').'">'.$langs->trans("BackToList").'</a>';

        // Ref du contrat
        print '<tr><td width="25%">'.$langs->trans("Ref").'</td><td colspan="3">';
     //   print $form->showrefnav($object, 'ref_interne', $linkback, 1, 'ref', 'ref', '');
        print "</td></tr>";
