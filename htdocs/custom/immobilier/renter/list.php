...
foreach($object->fields as $key => $val)
{
	if ($val['searchall']) $fieldstosearchall['s.'.$key]=$val['label'];
}
...
foreach($object->fields as $key => $val)
{
	...
	if (! empty($val['visible'])) $arrayfields['s.'.$key]=array('label'=>$val['label'], 'checked'=>(($val['visible']<0)?0:1), 'enabled'=>$val['enabled'], 'position'=>$val['position']);
}
...
$help_url='';
$title = $langs->trans($langs->transnoentitiesnoconv("ListRenters"));
...
$sql.= " FROM ".MAIN_DB_PREFIX.$object->table_element." as s";
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."immorenter_extrafields as ef on (s.rowid = ef.fk_object)";
$sql .= " LEFT JOIN " . MAIN_DB_PREFIX . "societe as so";
$sql .= " ON (s.fk_soc = so.rowid)";
$sql .= " LEFT JOIN " . MAIN_DB_PREFIX . "c_civility as civ";
$sql .= " ON s.civilite = civ.code";
if ($object->ismultientitymanaged == 1) $sql.= " WHERE s.entity IN (".getEntity('immorenter').")";
else $sql.=" WHERE 1 = 1";
...
if ($num == 1 && ! empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all)
{
	...
	$id = $obj->rowid;
	header("Location: ".DOL_URL_ROOT.'/immobilier/renter/card.php?id='.$id);
	exit;
}
...
foreach($object->fields as $key => $val)
{
	...
	if (! empty($arrayfields['s.'.$key]['checked'])) print '<td class="liste_titre'.($align?' '.$align:'').'"><input type="text" class="flat maxwidth75" name="search_'.$key.'" value="'.dol_escape_htmltag($search[$key]).'"></td>';
}
...
foreach($object->fields as $key => $val)
{
	...
	if (! empty($arrayfields['s.'.$key]['checked'])) print getTitleFieldOfList($arrayfields['s.'.$key]['label'], 0, $_SERVER['PHP_SELF'], 's.'.$key, '', $param, ($align?'class="'.$align.'"':''), $sortfield, $sortorder, $align.' ')."\n";
}
...
while ($i < min($num, $limit))
{
	...
	foreach($object->fields as $key => $val)
	{
		...
		if (! empty($arrayfields['s.'.$key]['checked']))
		{
			...
			{
				if (! $i) $totalarray['pos'][$totalarray['nbfield']]='s.'.$key;
				$totalarray['val']['s.'.$key] += $obj->$key;
			}
		}
	}
