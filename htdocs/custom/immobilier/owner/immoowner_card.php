...
$hookmanager->initHooks(array('immoownercard'));     // Note that conf->hooks_modules contains array
...
if (empty($reshook))
{
	...
	$backurlforlist = dol_buildpath('/immobilier/owner/immoowner_list.php',1);
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create')))
{
	...
	dol_fiche_head($head, 'card', $langs->trans("ImmoOwner"), -1, 'immoowner@immobilier');
	...
	// Object card
	// ------------------------------------------------------------
	$linkback = '<a href="' .dol_buildpath('/immobilier/immoowner_list.php',1) . '?restore_lastsearch_values=1' . (! empty($socid) ? '&socid=' . $socid : '') . '">' . $langs->trans("BackToList") . '</a>';
	...
	*/
	...
	if ($action != 'presend')
	{
		...
		*/
	    ...
	    $morehtmlright = '<a href="'.dol_buildpath('/immobilier/immoowner_info.php', 1).'?id='.$object->id.'">';
	    ...
	    $somethingshown = $formactions->showactions($object, 'immoowner', $socid, 1, '', $MAXEVENT, '', $morehtmlright);
