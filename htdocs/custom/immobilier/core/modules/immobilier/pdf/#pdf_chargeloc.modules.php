...
dol_include_once ( '/adherents/class/immoowner.class.php' );
...
class pdf_quittance extends ModelePDFImmobilier {
	 ...
	 */
	function write_file($immorent, $outputlangs, $file, $socid, $courrier) {
		...
		if (! is_object ( $immorent )) {
			$id = $immorent;
			$immorent = new ImmoRent ( $this->db );
			$ret = $immorent->fetch ( $id );
		}
		
		// dol_syslog ( "pdf_quittance::debug immorent=" . var_export ( $immorent, true ) );
		...
		if (file_exists ( $dir )) {
			...
			$pdf->SetTitle ( $outputlangs->convToOutputCharset ( $immorent->nom ) );
			$pdf->SetSubject ( $outputlangs->transnoentities ( "Chargeloc" ) );
			$pdf->SetCreator ( "Dolibarr " . DOL_VERSION . ' (Immobilier module)' );
			$pdf->SetAuthor ( $outputlangs->convToOutputCharset ( $user->fullname ) );
			$pdf->SetKeyWords ( $outputlangs->convToOutputCharset ( $immorent->nom ) . " " . $outputlangs->transnoentities ( "Document" ) );
			...
			$immorenter = new ImmoRenter ( $this->db );
			$result = $immorenter->fetch ( $immorent->locataire_id );
			
			$immoowner = new ImmoOwner ( $this->db );
			$result = $immoowner->fetch ( $immorent->proprietaire_id );
			
			$local = new Local ( $this->db );
			$result = $local->fetch ( $immorent->local_id );
			
			$immopayment = new ImmoPayment ( $this->db );
			$result = $immopayment->fetch_by_loyer ( $immorent->id );
			
			if (! empty ( $immorent->id )) {
				// New page
				$pdf->AddPage ();
				$pagenb ++;
				...
				$this->str = $immoowner->getFullName ( $outputlangs ) . "\n";
				$this->str .= $immoowner->address . "\n";
				$this->str .= $immoowner->zip . ' ' . $immoowner->town;
				$this->str .= ' - ' . $immoowner->country . "\n\n";
				if ($immoowner->phone) {
					$this->str .= $outputlangs->transnoentities ( 'Téléphone' ) . ' ' . $immoowner->phone . "\n";
				}
				if ($immoowner->fax) {
					$this->str .= $outputlangs->transnoentities ( 'Fax' ) . ' ' . $immoowner->fax . "\n";
				}
				if ($immoowner->email) {
					$this->str .= $outputlangs->transnoentities ( 'EMail' ) . ' ' . $immoowner->email . "\n";
				}
				if ($immoowner->url) {
					$this->str .= $outputlangs->transnoentities ( 'Url' ) . ' ' . $immoowner->url . "\n";
				}
				...
				$pdf->SetXY ( $posX, $posY );
				$pdf->SetFont ( pdf_getPDFFont ( $outputlangs ), '', 13 );
				$this->str = $immorenter->nom . "\n";
				$this->str .= $local->nom . "\n";
				if (! empty ( $immorenter->adresse )) {
					$this->str .= $immorenter->adresse . "\n";
				} else {
				...
				$text .= "\n";
				$text .= 'Fait à ' . $immoowner->town . ' le ' . dol_print_date ( dol_now (), 'daytext' ) . "\n";			
				...
				$sql .= " FROM " . MAIN_DB_PREFIX . "immobilier_immorent";
				$sql .= " WHERE local_id=" . $immorent->local_id . " AND locataire_id=" . $immorent->locataire_id;
