...
/**
 *   	\file       immoproperty_type_card.php
 *		\ingroup    immobilier
 *		\brief      Page to create/edit/view immoproperty_type
 */
...
dol_include_once('/immobilier/class/immoproperty_type.class.php');
dol_include_once('/immobilier/lib/immoproperty_type.lib.php');
...
$hookmanager->initHooks(array('immoproperty_typecard'));     // Note that conf->hooks_modules contains array
...
if (empty($reshook))
{
	...
	$backurlforlist = dol_buildpath('/immobilier/property/immoproperty_type_list.php',1);
	...
	$trackid='immoproperty_type'.$object->id;
...
if ($action == 'create')
{
	print load_fiche_titre($langs->trans("NewObject", $langs->transnoentitiesnoconv("ImmoProperty_Type")));
...
{
	print load_fiche_titre($langs->trans("ImmoPropertyType"));
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create')))
{
	...
	dol_fiche_head($head, 'card', $langs->trans("ImmoProperty_Type"), -1, 'immoproperty_type@immobilier');
	...
	if ($action == 'delete')
	{
	    $formconfirm = $form->formconfirm($_SERVER["PHP_SELF"] . '?id=' . $object->id, $langs->trans('DeleteImmoProperty_Type'), $langs->trans('ConfirmDeleteImmoProperty_Type'), 'confirm_delete', '', 0, 1);
	}
	...
	// Object card
	// ------------------------------------------------------------
	$linkback = '<a href="' .dol_buildpath('/immobilier/property/immoproperty_type_list.php',1) . '?restore_lastsearch_values=1' . (! empty($socid) ? '&socid=' . $socid : '') . '">' . $langs->trans("BackToList") . '</a>';
	...
	*/
	...
	if ($action != 'presend')
	{
		...
		*/
	    ...
	    $linktoelem = $form->showLinkToObjectBlock($object, null, array('immoproperty_type'));
	    ...
	    $morehtmlright = '<a href="'.dol_buildpath('/immobilier/property/immoproperty_type_info.php', 1).'?id='.$object->id.'">';
	    ...
	    $somethingshown = $formactions->showactions($object, 'immoproperty_type', $socid, 1, '', $MAXEVENT, '', $morehtmlright);
