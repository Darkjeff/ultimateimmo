...
if (empty($reshook))
{
	...
	$permissiontoadd = $user->rights->immobilier->create;
	$permissiontodelete = $user->rights->immobilier->delete;
	$backurlforlist = dol_buildpath('/immobilier/immoproperty_list.php',1);
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create')))
{
	...
	dol_fiche_head($head, 'card', $langs->trans("ImmoProperty"), -1, 'immoproperty@immobilier');
	...
	// Object card
	// ------------------------------------------------------------
	$linkback = '<a href="' .dol_buildpath('/immobilier/immoproperty_list.php',1) . '?restore_lastsearch_values=1' . (! empty($socid) ? '&socid=' . $socid : '') . '">' . $langs->trans("BackToList") . '</a>';
	...
	*/
	...
	if ($action != 'presend')
	{
		...
		*/
	    ...
	    $morehtmlright = '<a href="'.dol_buildpath('/immobilier/immoproperty_info.php', 1).'?id='.$object->id.'">';
