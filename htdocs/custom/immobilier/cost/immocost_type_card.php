...
/**
 *   	\file       immocost_type_card.php
 *		\ingroup    immobilier
 *		\brief      Page to create/edit/view immocost_type
 */
...
dol_include_once('/immobilier/lib/immocost_type.lib.php');
...
$contextpage= GETPOST('contextpage','aZ')?GETPOST('contextpage','aZ'):'immocost_typecard';   // To manage different context of search
...
$hookmanager->initHooks(array('immocost_typecard','globalcard'));     // Note that conf->hooks_modules contains array
...
if (empty($reshook))
{
	...
	$backurlforlist = dol_buildpath('/immobilier/cost/immocost_type_list.php',1);
	...
	$trackid='immocost_type'.$object->id;
...
if ($action == 'create')
{
	print load_fiche_titre($langs->trans("NewObject", $langs->transnoentitiesnoconv("ImmoCost_Type")));
...
{
	print load_fiche_titre($langs->trans("ImmoCost_Type"));
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create')))
{
	...
	dol_fiche_head($head, 'card', $langs->trans("ImmoCost_Type"), -1, 'immocost_type@immobilier');
	...
	if ($action == 'delete')
	{
	    $formconfirm = $form->formconfirm($_SERVER["PHP_SELF"] . '?id=' . $object->id, $langs->trans('DeleteImmoCost_Type'), $langs->trans('ConfirmDeleteImmoCost_Type'), 'confirm_delete', '', 0, 1);
	}
	...
	// Object card
	// ------------------------------------------------------------
	$linkback = '<a href="' .dol_buildpath('/immobilier/cost/immocost_type_list.php',1) . '?restore_lastsearch_values=1' . (! empty($socid) ? '&socid=' . $socid : '') . '">' . $langs->trans("BackToList") . '</a>';
	...
	*/
	...
	if ($action != 'presend')
	{
		...
		*/
	    ...
	    $linktoelem = $form->showLinkToObjectBlock($object, null, array('immocost_type'));
	    ...
	    $morehtmlright = '<a href="'.dol_buildpath('/immobilier/cost/immocost_type_info.php', 1).'?id='.$object->id.'">';
	    ...
	    $somethingshown = $formactions->showactions($object, 'immocost_type', $socid, 1, '', $MAXEVENT, '', $morehtmlright);
