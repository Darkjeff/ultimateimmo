...
/**
 *  \file       immopayment_document.php
 *  \ingroup    immobilier
 *  \brief      Tab for documents linked to ImmoPayment
 */
...
$hookmanager->initHooks(array('immopaymentdocument'));     // Note that conf->hooks_modules contains array
...
$title=$langs->trans("ImmoPayment").' - '.$langs->trans("Files");
$help_url='';
...
if ($object->id)
{
	...
	dol_fiche_head($head, 'document', $langs->trans("ImmoPayment"), -1, 'immopayment@immobilier');
	...
	// Object card
	// ------------------------------------------------------------
	$linkback = '<a href="' .dol_buildpath('/immobilier/immopayment_list.php',1) . '?restore_lastsearch_values=1' . (! empty($socid) ? '&socid=' . $socid : '') . '">' . $langs->trans("BackToList") . '</a>';
	...
	//$relativepathwithnofile='immopayment/' . dol_sanitizeFileName($object->id).'/';
	$relativepathwithnofile='immopayment/' . dol_sanitizeFileName($object->ref).'/';
