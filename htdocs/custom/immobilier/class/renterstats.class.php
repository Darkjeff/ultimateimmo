...
/**
 *	\file       htdocs/custom/immobilier/class/renterstats.class.php
 *	\ingroup    immobilier
...
class RenterStats extends Stats
{
	...
	function getNbByYear()
	{
		...
		$sql = "SELECT date_format(p.datec,'%Y') as dm, count(*)";
		$sql.= " FROM ".$this->from;
		//if (!$user->rights->societe->client->voir && !$user->societe_id) $sql.= ", ".MAIN_DB_PREFIX."societe_commerciaux as sc";
	...
	function getAllByYear()
	{
		...
		$sql = "SELECT date_format(p.datec,'%Y') as year, count(*) as nb, sum(".$this->field.") as total, avg(".$this->field.") as avg";
		$sql.= " FROM ".$this->from;
		//if (!$user->rights->societe->client->voir && !$this->socid) $sql.= ", ".MAIN_DB_PREFIX."societe_commerciaux as sc";
		$sql.= " WHERE ".$this->where;
		$sql.= " GROUP BY year";
