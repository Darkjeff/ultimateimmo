...

{
	...
	*/
	...
	$loyer->CommCharge = GETPOST ( 'CommCharge');
	$loyer->echeance = $dateech;
...
if ($id > 0) {
	...
	print '<tr><td width="25%">' . $langs->trans ( "NomLoyer" ) . '</td>';
	...
	print '<tr><td>' . $langs->trans ( "loy" ) . '</td>';
	...
	print '<tr><td>' . $langs->trans ( "charge_ex" ) . '</td>';
	print '<td><input name="charge_ex" size="10" value="' . $loyer->charge_ex . '"</td>';
	print '<tr><td>' . $langs->trans ( "CommCharge" ) . '</td>';


 
        print '<td>';
        
            $doleditor = new DolEditor('CommCharge', $loyer->CommCharge, 600, 200, 'dolibarr_notes', 'In', false, true, true, ROWS_8, '100');
	...
	print "<a class=\"butAction\" href=\"" . DOL_URL_ROOT . "/custom/immobilier/loyer/fiche_loyer.php?id=$loyer->id&amp;action=update\">" . $langs->trans ( "Modify" ) . "</a>";
	
	// Emettre paiement
	if ($object->paye == 0 && ((price2num ( $loyer->montant_tot ) < 0 && round ( $resteapayer ) < 0) || (price2num ( $loyer->montant_tot ) > 0 && round ( $resteapayer ) > 0))) {
		print "<a class=\"butAction\" href=\"" . DOL_URL_ROOT . "/custom/immobilier/loyer/fiche_paiement.php?id=$id&amp;action=create\">" . $langs->trans ( "DoPayment" ) . "</a>";
	}
	...
	// Delete
	print "<a class=\"butActionDelete\" href=\"" . DOL_URL_ROOT . "/custom/immobilier/loyer/fiche_loyer.php?id=".$id."&amp;action=delete\">" . $langs->trans ( "Delete" ) . "</a>";
	...
	print '<a class="butAction" href="' . $_SERVER ['PHP_SELF'] . '?action=chargeloc&id=' . $id . '">' . $langs->trans ( 'GenererCharge' ) . '</a>';
