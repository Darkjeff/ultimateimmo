...
if (empty($reshook))
{
	...
	$backurlforlist = dol_buildpath('/immobilier/property/immoproperty_list.php',1);
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create')))
{
	...
	dol_fiche_head($head, 'card', $langs->trans("ImmoProperty"), -1, 'immobilier@immobilier');
	...
	// Object card
	// ------------------------------------------------------------
	$linkback = '<a href="' .dol_buildpath('/immobilier/property/immoproperty_list.php',1) . '?restore_lastsearch_values=0' . (! empty($socid) ? '&socid=' . $socid : '') . '">' . $langs->trans("BackToList") . '</a>';
	...
	*/
	...
	if ($action != 'presend')
	{
		...
		$morehtmlright = '<a href="'.dol_buildpath('/immobilier/property/immoproperty_info.php', 1).'?id='.$object->id.'">';
	...
	$diroutput = $conf->immobilier->dir_output.'/property';
