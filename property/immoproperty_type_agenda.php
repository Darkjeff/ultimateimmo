...
if ($object->id > 0)
{
	...
	dol_fiche_head($head, 'agenda', $langs->trans("ImmoProperty_Type"), -1, 'immobilier@immobilier');

	// Object card
	// ------------------------------------------------------------
	$linkback = '<a href="' .dol_buildpath('/immobilier/immoproperty_type_list.php',1) . '?restore_lastsearch_values=1' . (! empty($socid) ? '&socid=' . $socid : '') . '">' . $langs->trans("BackToList") . '</a>';
